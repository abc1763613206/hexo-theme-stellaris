...
module.exports = (ctx) =>
  function (args) {
    args = ctx.args.map(
      args,
      ["width", "height", "bg", "download", "padding", "fancybox"],
      ["src", "alt"]
    );
    var style = "";
    if (args.width) {
    ...
    var fancybox = false;
    if (
      ctx.theme.config.plugins.fancybox &&
      ctx.theme.config.plugins.fancybox.enable
    ) {
      // 主题配置
      if (
        ctx.theme.config.tag_plugins.image &&
        ctx.theme.config.tag_plugins.image.fancybox
      ) {
