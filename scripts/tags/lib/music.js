 ...
 * {% music auto:https://music.163.com/#/song?id=18520488 [loop:all/one/none] %} 
 * # server、type、id 为必填项，请根据 https://github.com/metowolf/MetingJS 进行替换
 * {% music server:type:id [loop:all/one/none] %} 
 * 示例： {% music netease:song:18520488 loop:all %}
 * 
...
module.exports = (ctx) => (args) => {
    args = ctx.args.map(args, ['name', 'artist', 'cover', 'loop', 'auto', 'server', 'type', 'id', 'url'], ['external'])
    ...
    return `<div class="tag-plugin ds-music" metingargs="${Buffer.from(metingArgs).toString('base64')}"></div>`
