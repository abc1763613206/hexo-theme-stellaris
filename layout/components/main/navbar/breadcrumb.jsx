...
const BreadNav = (props) => (
  <div className='bread-nav fs12'>{props.children}</div>
)

const Breadcrumb = (props) => {
  const { theme, page, config, date, date_xml, url_for, __, md_link, md_text } =
    props
  ...
  if (page.layout === 'post') {
    ...
    return (
      <BreadNav>
        <div className='left'>
          <div id='breadcrumb'>
            <a className='cap breadcrumb' href={url_for(config.root)}>
              {home_title}
            </a>
            <span className='sep'></span>
            <a
              className='cap breadcrumb'
    ...
    const Menu = (props) => {
      ...
      if (
        page.menu_id &&
        theme.sidebar.menu[page.menu_id] &&
        md_link(theme.sidebar.menu[page.menu_id])
      ) {
        const url = url_for(md_link(theme.sidebar.menu[page.menu_id]))
        nodes.push(url)
        return (
          <a className='cap breadcrumb' id='menu' href={url}>
      ...
      if (proj) {
        ...
        if (nodes.includes(url_proj) === false) {
          return (
            <Fragment>
              <span className='sep'></span>
              <a className='cap breadcrumb' id='proj' href={url_proj}>
    ...
    const Repo = (props) => {
      ...
      if (repo) {
        return (
          <>
            <div
              className='right ghrepo ds-ghinfo'
              api={theme.api_host.ghapi + '/repos/' + repo}
            >
              <a className='repo-link bold' href={'https://github.com/' + repo}>
                <svg
                  ...
                  height='1em'
                  fill='currentColor'
                  style={{ userSelect: 'none', verflow: 'visible' }}
                >
                  <path
                    fillRule='evenodd'
              ...
              >
                ...
                <span type='text' id='stargazers_count'>
                  0
                </span>
                <span>stars</span>
              </a>
      ...
      <BreadNav>
        <div className='left'>
          <div id='breadcrumb'>
            ...
            <span className='sep'></span>
            <a className='cap breadcrumb' href={url_for(page.path)}>
