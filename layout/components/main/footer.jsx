              ...
              siteMapGroups.push(
                <div className='sitemap-group' key={group}>
                  ...
                  {(() => {
                    ...
                    items.forEach((item) => {
                      siteMapItems.push(
                        <a href={url_for(md_link(item))} key={item}>
                          {__(md_text(item))}
                        </a>
                      )
                    })
  ...
  const FooterContent = (props) => {
    ...
    return (
      <div className='text'>
        ...
        {source
          ? parse(
              markdown(__('footer.info_open_source', author, using, source))
            )
          : parse(markdown(__('footer.info_not_open_source', author, using)))}
        <div>
          {__('footer.powered_by_1')}
        ...
        {icp &&
          (() => {
            if (typeof icp == 'string' && icp.constructor === String) {
              ...
              const elements = []
              icp.forEach((item, i) => {
                elements.push(<p key={item}>{parse(markdown(item))}</p>)
              })
              return elements
            } else {
              return <></>
