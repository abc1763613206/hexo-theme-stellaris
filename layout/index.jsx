...
const LayoutWikiCard = (props) => {
  ...
  return (
    <a
      className={className}
      href={url_for(proj.homepage.link || proj.homepage.path)}
      key={proj.path}
    >
...
const LayoutPosts = (props) => {
  const PostCards = (props) => {
    ...
    if (is_home()) {
      if (page.current === 1) {
        // Pinned posts
        const pinned = site.posts
          .filter((post) => post.pin !== undefined)
          .sort(
            (config.index_generator && config.index_generator.order_by) ||
              '-date'
          )
        pinned.forEach((post) => {
